export function Location() {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      xmlSpace="preserve"
      width="100%"
      height="100%"
      viewBox="0 0 64 64"
    >
      <linearGradient
        id="a"
        x1="9.902"
        x2="54.098"
        y1="32"
        y2="32"
        gradientUnits="userSpaceOnUse"
      >
        <stop offset="0" stopColor="#00c0ff"></stop>
        <stop offset="1" stopColor="#5558ff"></stop>
      </linearGradient>
      <path
        fill="url(#a)"
        d="M31.31 53.947a1.017 1.017 0 0 0 1.38 0c6.498-6.24 19.691-20.702 19.597-29.66a20.287 20.287 0 1 0-40.574 0c-.103 8.943 13.105 23.437 19.597 29.66m-7.953-29.66c.46-11.457 16.828-11.453 17.286 0a8.643 8.643 0 0 1-17.286 0m30.74 28.93c-.076 9.044-44.119 9.044-44.195 0 0-3.241 5.712-5.192 12.175-6.122.454.55 1.105 1.254 1.59 1.8-8.013.96-11.764 3.041-11.764 4.322 0 1.96 7.833 4.802 20.097 4.802s20.097-2.841 20.097-4.802c0-1.28-3.751-3.361-11.764-4.322l.7-.78c.3-.34.6-.68.89-1.02 6.463.93 12.175 2.88 12.175 6.122z"
        data-original="url(#a)"
      ></path>
    </svg>
  );
}
